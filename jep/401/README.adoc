...
In terms of simplicity, proposal aims to use only one extension to do a full replacement of the header. As a follow up, each mentioned section of the header could have its own extension to override only a specific part of the header.

=== Extensibility

Jenkins users will be able to interact with the `Header` extension as follows in case want to customize the header of their instances via plugin:

* Extend `Header` and provide an implementation of the priority method giving to this header the max possible value (via ordinal value)
