...
```
    public static String getHeaderLabel(){
        return "Hello World!"; // This label content could be retrieved programatically. Not coded in aims of simplicity.
    }
```

* Provide the jelly files to override the core ones: headerContent, preHeader and/or postHeader. For that purpose, use the common location convention. For the previous example: `src/main/resources/org/jenkinsci/plugins/custom/header/CustomHeader/`. Retrieve the customizable label to be rendered with the username.
